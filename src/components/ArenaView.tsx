  ...
  const handleRun = async (script: ArenaScript) => {
    ...
    try {
      if (script.language === 'html') {
        ...
        const result = await executeCode(script.language, script.version, script.code);
        setOutputs(prev => ({ ...prev, [script.id]: result.run?.output || result.compile?.output || 'No output' }));
      }
    } catch (err: any) {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-[#111218] text-slate-300 p-4 sm:p-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-2xl font-bold mb-2 text-white tracking-tight">Script Arena</h1>
        <p className="text-slate-500 mb-8 text-sm">Discover, test, and compare community scripts.</p>

        <div className="grid gap-6">
          {scripts.map(script => (
            <div key={script.id} className="bg-[#101218] border border-white/5 rounded-lg overflow-hidden transition-all hover:border-white/10 shadow-lg">
              <div className="p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row justify-between items-start mb-4 gap-3 sm:gap-0">
                  <div>
                    ...
                    <div className="flex items-center gap-3 text-[11px] font-medium text-slate-500 uppercase tracking-wider">
                      <span className="flex items-center gap-1 text-purple-400"><Code2 size={14}/> {script.language}</span>
                ...
                {outputs[script.id] && (
                  <div className="mt-5 p-4 bg-black/40 border border-white/5 rounded text-[12px] font-mono whitespace-pre-wrap text-slate-400">
                    <span className="text-slate-500 text-[10px] uppercase font-bold tracking-widest mb-2 block">Output:</span>
