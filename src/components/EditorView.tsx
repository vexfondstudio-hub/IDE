...
const DEFAULT_SNIPPETS: Record<string, string> = {
  javascript: 'console.log("Hello, Joker!");',
  python: 'print("Hello, Joker!")',
  cpp: '#include <iostream>\n\nint main() {\n    std::cout << "Hello, Joker!" << std::endl;\n    return 0;\n}',
  java: 'class Main {\n    public static void main(String[] args) {\n        System.out.println("Hello, Joker!");\n    }\n}',
  csharp:
    'using System;\n\nclass Program {\n    static void Main() {\n        Console.WriteLine("Hello, Joker!");\n    }\n}',
  rust: 'fn main() {\n    println!("Hello, Joker!");\n}',
  lua: 'print("Hello, Joker!")',
  html: "<!DOCTYPE html>\n<html>\n<head>\n  <style>\n    body { font-family: system-ui; color: white; background: #111218; padding: 20px; }\n    h1 { color: #A855F7; }\n  </style>\n</head>\n<body>\n  <h1>Hello, Joker!</h1>\n</body>\n</html>",
};

export function EditorView() {
  const [language, setLanguage] = useState(LANGUAGES[0]);
  const [editorEngine, setEditorEngine] = useState<
    "codemirror" | "monaco" | "ace" | "simple" | "codejar"
  >("codemirror");
  ...
  useEffect(() => {
    currentLangRef.current = language.id;
  }, [language.id]);
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (
        ...
        finalOutput.toLowerCase().includes("exception")
      ) {
  ...
  const handleSelectSnippet = (snippet: Snippet) => {
    ...
    const lang =
      LANGUAGES.find((l) => l.id === snippet.languageId) || LANGUAGES[0];
    setLanguage(lang);
  };
  ...
  const aiAutocomplete = async (
    ...
    if (!context.explicit && word.text.length < 2) {
      return null;
    }
  ...
  const extensions = useMemo(() => {
    return [
      ...
      autocompletion({ override: [aiAutocomplete] }),
    ];
  }, [language.id]);

  return (
    <div className="flex h-full bg-[#111218] text-slate-300">
      <FileExplorer
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-3 bg-[#101218] border-b border-white/5 gap-3 sm:gap-0 z-20">
          <div className="flex flex-wrap items-center gap-2 sm:gap-4 w-full sm:w-auto justify-start">
            <button
              ...
              className="md:hidden hover:bg-white/10 p-1.5 rounded transition-colors text-slate-400 hover:text-white"
            >
              <Menu size={18} />
            </button>
            <div className="relative" ref={langMenuRef}>
              ...
              >
                ...
                <ChevronDown
                  size={14}
                  className={`transition-transform duration-200 ${isLangMenuOpen ? "rotate-180 text-purple-400" : "text-slate-500"}`}
                />
              </button>
                ...
                <div className="absolute top-full left-0 mt-1 w-48 bg-[#1A1D24] border border-white/10 rounded-lg shadow-xl overflow-hidden py-1 z-50">
                  {LANGUAGES.map((lang) => (
                    <button
                      ...
                      onClick={() => {
                        setLanguage(lang);
                        setCode(DEFAULT_SNIPPETS[lang.id] || "");
                        setIsLangMenuOpen(false);
                      }}
            ...
            <div className="relative" ref={engineMenuRef}>
              <button
                ...
                className="flex items-center justify-between gap-2 bg-black/40 text-slate-300 px-3 py-1.5 rounded text-xs font-medium outline-none border border-white/5 hover:border-blue-500/50 hover:bg-white/5 transition-colors w-32"
              >
                ...
                <ChevronDown
                  size={14}
                  className={`transition-transform duration-200 ${isEngineMenuOpen ? "rotate-180 text-blue-400" : "text-slate-500"}`}
                />
              </button>
                ...
                <div className="absolute top-full left-0 mt-1 w-48 bg-[#1A1D24] border border-white/10 rounded-lg shadow-xl overflow-hidden py-1 z-50">
                  {(
                    ...
                    <button
                      ...
                      className={`w-full text-left px-4 py-2 text-xs font-medium transition-colors ${editorEngine === engine ? "bg-blue-600/20 text-blue-400 border-l-2 border-blue-500" : "text-slate-300 hover:bg-white/5 border-l-2 border-transparent"}`}
                    >
                      {engine === "codemirror"
                        ? "CodeMirror 6"
                        : engine === "monaco"
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 scrollbar-hide">
            <label
              className="flex items-center gap-2 hover:bg-white/5 px-3 py-1.5 rounded transition-colors text-xs font-medium text-slate-400 hover:text-white cursor-pointer shrink-0"
              title="Open File"
            >
              <Upload size={14} />
              Open
              <input
                ...
                accept=".js,.py,.cpp,.cs,.java,.rs,.txt"
              />
            </label>
            <button
              onClick={handleLocalDownload}
              className="flex items-center gap-2 hover:bg-white/5 px-3 py-1.5 rounded transition-colors text-xs font-medium text-slate-400 hover:text-white shrink-0"
            ...
            {user ? (
              ...
              <button
                ...
                className="flex items-center gap-2 hover:bg-white/5 px-3 py-1.5 rounded transition-colors text-xs font-medium text-purple-400 shrink-0"
              >
                Sign in to Save
              </button>
            )}
        ...
        <div className="flex-1 flex flex-col md:flex-row min-h-0">
              ...
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 glass rounded-lg p-4 w-[90%] sm:w-[400px] shadow-2xl border-purple-500/30 z-40 max-h-[80vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    ...
                    className="text-slate-500 hover:text-white transition-colors text-lg font-bold w-6 h-6 flex items-center justify-center rounded hover:bg-white/10"
                  >
                    &times;
                  </button>
                </div>
                ...
                {aiHint.suggestion && (
                  <div className="bg-black/50 p-3 rounded border border-white/5 mb-3 overflow-x-auto">
                    <pre className="text-[11px] sm:text-xs font-mono text-purple-200 whitespace-pre-wrap">
                      {aiHint.suggestion}
                    </pre>
                  </div>
                )}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => setAiHint(null)}
                    className="flex-1 text-slate-400 hover:bg-white/5 text-[11px] px-3 py-2 rounded transition-colors text-center border border-transparent hover:border-white/10"
                  >
                    Dismiss
                  </button>
                </div>
          ...
          <div className="w-full h-64 md:h-full md:w-[400px] flex flex-col bg-[#0B0C10] shrink-0 z-10 border-t md:border-t-0 md:border-l border-white/5">
            <div className="p-3 border-b border-white/5 text-[10px] font-bold text-slate-500 uppercase tracking-widest flex justify-between items-center bg-[#101218]">
              ...
              <button
                onClick={() => setOutput("")}
                className="text-slate-400 hover:text-white transition-colors bg-white/5 px-2 py-1 rounded"
              >
                Clear
              </button>
            </div>
          ...
          <div className="bg-[#101218] p-6 rounded-xl border border-white/10 shadow-2xl w-96">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 text-xs font-medium bg-purple-600 hover:bg-purple-500 text-white rounded transition-colors"
              >
                Send
              </button>
            </div>
